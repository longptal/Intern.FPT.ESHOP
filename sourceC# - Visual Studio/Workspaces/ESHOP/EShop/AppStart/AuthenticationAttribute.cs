...
namespace EShop
{
    public class AuthenticationFilter : IActionFilter
    {
        ...
        {
            var EToken = FilterContext.HttpContext.Request.Cookies["EJWT"];
            var EJWTEntity = JWTHandler.Decode(EToken);
            var CToken = FilterContext.HttpContext.Request.Cookies["CJWT"];
            var CJWTEntity = JWTHandler.Decode(CToken);
            if (EJWTEntity != null || CJWTEntity != null)
            {
                ...
                if (CJWTEntity != null)
                {
                    ...
                    CustomerEntity.Id = CJWTEntity.UserEntity.Id;
                    CustomerEntity.Username = CJWTEntity.UserEntity.Username;
                    CustomerEntity.Display = CJWTEntity.UserEntity.Display;
                }
                if (EJWTEntity != null)
                {
                    EmployeeEntity = new EmployeeEntity();
                    EmployeeEntity.Id = EJWTEntity.UserEntity.Id;
                    EmployeeEntity.Username = EJWTEntity.UserEntity.Username;
                    EmployeeEntity.Display = EJWTEntity.UserEntity.Display;
                }
                FilterContext.HttpContext.User = new MyPrincipal(EmployeeEntity, CustomerEntity);
                return;
            }
            //else
    ...
    public interface IJWTHandler : ITransientService
    {
        ...
        string CreateToken(CustomerEntity CustomerEntity);
        string CreateToken(EmployeeEntity EmployeeEntity);
    }
  ...
  <Exponent>AQAB</Exponent>
    ...
    <D></D>
        ...
        public string CreateToken(CustomerEntity CustomerEntity)
        {
            if (_signingCredentials == null) throw new BadRequestException("Server khong tao duoc token");
            var nowUtc = DateTime.UtcNow;
            var expires = nowUtc.AddDays(30);
        ...
        public string CreateToken(EmployeeEntity EmployeeEntity)
        {
            if (_signingCredentials == null) throw new BadRequestException("Server khong tao duoc token");
            var nowUtc = DateTime.UtcNow;
            var expires = nowUtc.AddDays(30);
