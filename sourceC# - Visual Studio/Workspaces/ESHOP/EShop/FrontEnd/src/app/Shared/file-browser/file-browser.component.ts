...
export class FileBrowserComponent implements OnInit, ControlValueAccessor {
  ...
  writeValue(obj: any): void {
    ...
    this.selectedFileEntities = files.map(f => {
      if (f == '') return;
      let fbe = new FileBrowserEntity();
      fbe.Src = f;
      fbe.IsSelected = true;
      return fbe;
    }).filter(f => f != null);
    if (this.fileMap == null) this.Search('/Files');
    else this.Search(this.fileMap.join('/'));
  }
  ...
  AddNewFolder() {
    let Directory = new DirectoryEntity();
  ...
  RenameFolder() {
    for (let file of this.fileBrowserEntities) {
  ...
  DeleteFolder() {
    let ConfirmBox = confirm('Are you sure about delete this folder?');
    if (ConfirmBox) {
      let Directory = new DirectoryEntity();
  ...
  PostNameFolder(FileEntity: FileBrowserEntity) {
    ...
    this.DirectoryService.RenameFolder(Directory).subscribe(res => {
      if (res) {
        let Index = this.fileBrowserEntities.indexOf(FileEntity);
  ...
  UploadImage(event) {
    ...
    if (fileList.length > 0) {
      ...
      let ConfirmBox = confirm('Are you sure about upload this image?');
      if (ConfirmBox) {
